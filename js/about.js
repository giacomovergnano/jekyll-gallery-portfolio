...
class Boids {
  constructor(opts = {}) {
    ...
    this.boids = Array(opts.boids || 50).fill(0).map(() => ({
      speed: [utils.rand(-25, 25), utils.rand(-25, 25)],
      position: [
        utils.rand(-quarterWidth, quarterWidth),
        utils.rand(-quarterHeight, quarterHeight)
      ],
      acceleration: [0, 0]
    }));
  }
  ...
  tick() {
    this.boids.forEach(boid => {
      let [sForceX, sForceY, cForceX, cForceY, aForceX, aForceY] = Array(
        6
      ).fill(0);
      let length;
      ...
      if (this.accelerationLimit) {
        let dist =
          boid.acceleration[0] * boid.acceleration[0] +
          boid.acceleration[1] * boid.acceleration[1];

        if (dist > this.accelerationLimit) {
          let ratio =
            this.accelerationLimitRoot /
            utils.hypot(boid.acceleration[0], boid.acceleration[1]);
      ...
      if (this.speedLimit) {
        let dist =
          boid.speed[0] * boid.speed[0] + boid.speed[1] * boid.speed[1];

        if (dist > this.speedLimit) {
          let ratio =
            this.speedLimitRoot / utils.hypot(boid.speed[0], boid.speed[1]);
          boid.speed[0] *= ratio;
          boid.speed[1] *= ratio;
        }
      }
  ...
  draw() {
    this.boids.forEach(boid => {
      ...
      boid.position[0] = x > halves.width
        ? -halves.width
        : -x > halves.width ? halves.width : x;
      boid.position[1] = y > halves.height
        ? -halves.height
        : -y > halves.height ? halves.height : y;
...
class Clocks {
  ...
  tick() {
    let now = new Date();
    this.el.textContent = `${now.getHours() %
      12}:${now.getMinutes()} ${now.getHours() >= 12 ? "PM" : "AM"}`;
  }
}
...
const boids = new Boids({
  ...
  accelerationLimit: 0.05, // Max acceleration per tick
...
function onMouseEnter(e) {
  boids.attractors.push([e.x, e.y, 150, 0.4]);
}
